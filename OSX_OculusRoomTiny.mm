...
OculusRoomTinyApp::OculusRoomTinyApp(OVRApp* nsapp)
    ...
    // Initial location
    EyePos(-9.0f, 1.6f, -9.0f),
    PreEyePos(-9.0f, 1.6f, -9.0f),
    FurthestPos(-8.0f, 1.6f, -8.0f),
...
bool  OculusRoomTinyApp::setupWindow()
{
    ...
    SetFullscreen(RenderParams, false); //true
    return true;
}
...
Vector3f OculusRoomTinyApp::Wallchecking(Vector3f EyePos, Vector3f PreEyePos)
{
    Vector3f corrPos = EyePos;
    //wall coordinates
    int coords[][2] = { {0, 5}, {2, 5}, {3, 5}, {4, 5}, {5, 5}, {5, 0}, {5, 2}, {5, 3}, {5, 4}, {5, 6}, {5, 7}, {5, 9}, {6, 4}, {7, 4}, {9, 4} };
    for (int k=0; k<15; k++) {
        float x = coords[k][0]*2 -9.0;
        float z = 9.0 - coords[k][1]*2 ;
        if (abs(EyePos.x - x) < 0.99 ) {
            if (PreEyePos.z < z-1.5 && EyePos.z > z-1.5) {
                corrPos.z = PreEyePos.z;
            }
            else if (PreEyePos.z > z+1.5 && EyePos.z < z+1.5) {
                corrPos.z = PreEyePos.z;
            }
        }
        if(abs(EyePos.z - z) < 0.99 ) {
            if (PreEyePos.x < x-1.5 && EyePos.x > x-1.5) {
                corrPos.x = PreEyePos.x;
            }
            else if (PreEyePos.x > x+1.5 && EyePos.x < x+1.5) {
                corrPos.x = PreEyePos.x;
            }
        }
    }
...
void OculusRoomTinyApp::OnIdle()
{
    ...
    if (MoveForward || MoveBack || MoveLeft || MoveRight)
    {
        ...
        else if (EyePos.x < -9.5 ) {
            EyePos.x = -9.5;
        }
        if (EyePos.z > 9.5 ) {
            EyePos.z = 9.5;
        }
        else if(EyePos.z < -9.5 ) {
            EyePos.z = -9.5;
        }
        
        EyePos = Wallchecking(EyePos, PreEyePos);

        //update the furthest point
        if (EyePos.x > FurthestPos.x) {
            FurthestPos.x = EyePos.x;
        }
        if (EyePos.z > FurthestPos.z) {
            FurthestPos.z = EyePos.z;
        }
        
    }
    
    //update the light position
    Scene.LightPos[0] = EyePos;
    //update the robot
    //Scene.World.Nodes[5]->SetPosition(EyePos);
    //Scene.World.Nodes[5]->Rotate(SFusion.GetOrientation()); // hmdOrient
    //push the wall
    Scene.World.Nodes[6]->SetPosition(FurthestPos);
    ...
    //robot!!!
    Scene.World.Nodes[5]->SetPosition(shiftedEyePos+forward);
